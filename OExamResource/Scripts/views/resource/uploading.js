$(function () {
    var par = {};
    $.sync.ajax.post("/Resource/Getload", par, function (data) {
        if (data.State) {
            $("#upload").setTemplateURL("/Scripts/template/resource/loading.htm");
            $("#upload").processTemplate(data.Result);
        }
    });
    $.sync.ajax.post("/Resource/Getcategory", par, function (data) {
        $("#slcategory").setTemplateURL("/Scripts/template/resource/category.htm");
        $("#slcategory").processTemplate(data.Result);
    });
    $("#slcategory").change(function () {
        par = {};
        par.cateid = $("#slcategory").val();
        $.sync.ajax.post("/Resource/Getslsub", par, function (data) {
            if (data.State) {
                $("#slsubject").setTemplateURL("/Scripts/template/resource/category.htm");
    ...
    $("#slsubject").change(function () {
        ...
        $.sync.ajax.post("/Resource/Getspe", par, function (data) {
            $("#slspeciality").setTemplateURL("/Scripts/template/resource/category.htm");
            $("#slspeciality").processTemplate(data.Result);
        });
    });
    $.sync.ajax.post("/Resource/Gettypelist", par, function (data) {
        $("#sltype").setTemplateURL("/Scripts/template/resource/category.htm");
    ...
    .change(function () {
        ...
        $.sync.ajax.post("/Resource/Typepic", par, function (data) {
    ...
    $("#btngoup").click(function () {
        window.location = "/Resource/Preview?hdId=" + $("#hdId").val() + "&hdType=" + $("#hdtype").val() + "&hdData=" + $("#Guidlist").val() + "&hdsearchname=" + $("#hdsearchname").val();
    });
